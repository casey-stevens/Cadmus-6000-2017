...
rooms.dat4 <- summarise(group_by(rooms.dat2, CK_Cadmus_ID, Clean.Room)
                        ,SiteArea = sum(Area))

#merge together analysis data with cleaned RBSA data
rooms.final <- left_join(rbsa.dat, rooms.dat4, by = "CK_Cadmus_ID")
...
item220.dat1 <- left_join(item220.dat, rooms.final)
...
item220.data <- weightedData(item220.merge[which(colnames(item220.merge) %notin% c("Clean.Room"
                                                                                 ,"SiteArea"
                                                                                 ,"Total.Area"
                                                                                 ,"CommonFloorArea"))])
