...
# item10.dat2 <- item10.dat1[which(!(item10.dat1$Wall.Type %in% c("Knee Wall","Masonry","Masonry (Basement)","Log","Adiabatic"))),]

#create "Alternative" category
item10.dat1$Wall.Type[which(item10.dat1$Wall.Type %in% c("Knee Wall","Masonry","Masonry (Basement)","Log","Adiabatic"))] <- "Alternative"
length(unique(item10.dat1$CK_Cadmus_ID))#491

# item10.wallType <- unique(item10.dat2[which(colnames(item10.dat2) %in% c("CK_Cadmus_ID", "Wall.Framing.Size"))])
# item10.wallType2 <- left_join(rbsa.dat, item10.wallType, by = "CK_Cadmus_ID")

#remove items have the datapoint was not asked for
item10.dat2 <- item10.dat1[which(item10.dat1$Wall.Framing.Size != "-- Datapoint not asked for --"),]
item10.dat2 <- item10.dat2[which(item10.dat2$`Wall.Cavity.Insulated?` != "Unknown"),]


###########################
# Cleaning Step: Set up unknown and N/A insulation thickness information in order to separate the # from the word "inches" in R
###########################
item10.dat2$Wall.Cavity.Insulation.Thickness.1[which(item10.dat2$Wall.Cavity.Insulation.Thickness.1 == "Unknown")] <- "Unknown Unknown"
item10.dat2$Wall.Cavity.Insulation.Thickness.1[which(item10.dat2$Wall.Cavity.Insulation.Thickness.1 == "N/A")] <- "N/A N/A"
item10.dat2$Wall.Cavity.Insulation.Thickness.1[which(is.na(item10.dat2$Wall.Cavity.Insulation.Thickness.1))] <- "N/A N/A"
item10.dat2$Wall.Cavity.Insulation.Thickness.2[which(item10.dat2$Wall.Cavity.Insulation.Thickness.2 == "Unknown")] <- "Unknown Unknown"
item10.dat2$Wall.Cavity.Insulation.Thickness.2[which(item10.dat2$Wall.Cavity.Insulation.Thickness.2 == "N/A")] <- "N/A N/A"
item10.dat2$Wall.Cavity.Insulation.Thickness.2[which(is.na(item10.dat2$Wall.Cavity.Insulation.Thickness.2))] <- "N/A N/A"

# add new ID variable for merging -- don't know if we need this
item10.dat2$count <- 1
item10.dat2$TMP_ID <- cumsum(item10.dat2$count)

## r-values ##
clean.insul1 <- unlist(strsplit(item10.dat2$Wall.Cavity.Insulation.Thickness.1, " "))
clean.insul2 <- as.data.frame(matrix(clean.insul1, ncol = 2, byrow = T), stringsAsFactors = F)
clean.insul1.1 <- cbind.data.frame("CK_Cadmus_ID" = item10.dat2$CK_Cadmus_ID
                                   , "TMP_ID" = item10.dat2$TMP_ID
                                   , clean.insul2)
dim(clean.insul1.1)

clean.insul2 <- unlist(strsplit(item10.dat2$Wall.Cavity.Insulation.Thickness.2, " "))
clean.insul2.1 <- cbind.data.frame("CK_Cadmus_ID" = item10.dat2$CK_Cadmus_ID
                                   , "TMP_ID" = item10.dat2$TMP_ID
...
#make into dataframe
item10.dat3 <- as.data.frame(left_join(item10.dat2, clean.insul, by = c("CK_Cadmus_ID", "TMP_ID"))
                           , stringsAsFactors = F) 
# warning here is OK
...
###########################
# rename columns
item10.dat3$inches1 <- as.numeric(as.character(item10.dat3$V1.x)) # warning here is OK
item10.dat3$inches2 <- as.numeric(as.character(item10.dat3$V1.y)) # warning here is OK

item10.dat3$rvalues1 <- item10.dat3$Wall.Cavity.Insulation.Type.1
item10.dat3$rvalues2 <- item10.dat3$Wall.Cavity.Insulation.Type.2

#check uniques
unique(item10.dat3$rvalues1)
unique(item10.dat3$rvalues2)

#fix names that are not in R value table
item10.dat3$rvalues1[which(item10.dat3$rvalues1 == "Fiberglass or mineral wool batts")] <- "Mineral wool batts"
item10.dat3$rvalues1[which(item10.dat3$rvalues1 == "Unknown fiberglass")]               <- "Unknown"
item10.dat3$rvalues2[which(item10.dat3$rvalues2 == "Extruded polystyrene (blue)")]      <- "Extruded polystyrene foam board"
item10.dat3$rvalues2[which(item10.dat3$rvalues2 == "N/A")]                              <- NA
###########################
# End cleaning step
###########################
...
for (i in 1:length(rvals$Type.of.Insulation)){
  item10.dat3$rvalues1[which(item10.dat3$rvalues1 == rvals$Type.of.Insulation[i])] <- rvals$`Avg..R-Value.Per.Inch`[i]
  item10.dat3$rvalues2[which(item10.dat3$rvalues2 == rvals$Type.of.Insulation[i])] <- rvals$`Avg..R-Value.Per.Inch`[i]
}
###########################
# End Replace Step
###########################

item10.dat4 <- item10.dat3
...
item10.dat4$rvalues1[which(is.na(item10.dat4$rvalues1))] <- 0
item10.dat4$rvalues1[which(item10.dat4$rvalues1 == "None")] <- 0
item10.dat4$rvalues2[which(is.na(item10.dat4$rvalues2))] <- 0

#QC the clean bulb values
unique(item10.dat4$rvalues1)
unique(item10.dat4$rvalues2)

item10.dat4$inches1[which(is.na(item10.dat4$inches1))] <- 0
item10.dat4$inches2[which(is.na(item10.dat4$inches2))] <- 0

#QC the clean bulb values -- There are NAs in each 
unique(item10.dat4$inches1)
unique(item10.dat4$inches2)
###########################
# End Cleaning step
###########################
...
item10.dat4$total.r.val <- (as.numeric(as.character(item10.dat4$rvalues1)) * item10.dat4$inches1) #+
# (as.numeric(as.character(item10.dat4$rvalues2)) * item10.dat4$inches2)

#caluclate u factors = inverse of Rvalue
item10.dat4$ufactor <- 1 / as.numeric(as.character(item10.dat4$total.r.val))
item10.dat4$ufactor <- as.numeric(as.character(item10.dat4$ufactor))

# replace inf with 0
item10.dat4$ufactor[which(item10.dat4$ufactor == "Inf")] <- 0

#make area numeric
item10.dat4$Wall.Area <- as.numeric(as.character(item10.dat4$Wall.Area))

#weight the u factor per home
weightedU <- summarise(group_by(item10.dat4, CK_Cadmus_ID)
...
wall.unique <- unique(item10.dat4[,which(colnames(item10.dat4)=="CK_Cadmus_ID" | 
                                         colnames(item10.dat4)=="BuildingType" |
                                         colnames(item10.dat4)=="HomeYearBuilt")])

item10.dat5 <- left_join(weightedU, wall.unique, by = "CK_Cadmus_ID")
...
item10.dat5$aveRval[which(item10.dat5$aveRval == 0)] <- NA

#get indicators for ceilings that are not insulated and rvalues that are NA
wall.ins.ind  <- item10.dat3$CK_Cadmus_ID[which(item10.dat3$`Wall.Cavity.Insulated?` == "No")] 
wall.r.NA.ind <- item10.dat5$CK_Cadmus_ID[which(is.na(item10.dat5$aveRval))]
...
item10.dat5$aveRval[which(item10.dat5$CK_Cadmus_ID %in% no.insulation)] <- 0
############################################################################################################
## END Process
############################################################################################################

wall.final <- item10.dat5

#Bin R values -- SF only
wall.final$rvalue.bins <- NA
wall.final$rvalue.bins[which(wall.final$aveRval == 0)] <- "R0"
wall.final$rvalue.bins[which(wall.final$aveRval > 0  & wall.final$aveRval < 11)]    <- "R1.R10"
wall.final$rvalue.bins[which(wall.final$aveRval > 11   & wall.final$aveRval < 17)]  <- "R11.R16"
wall.final$rvalue.bins[which(wall.final$aveRval > 17  & wall.final$aveRval < 23)]  <- "R17.R22"
wall.final$rvalue.bins[which(wall.final$aveRval > 23)] <- "RGT22"
unique(wall.final$rvalue.bins)

##cast data
wall.final$count <- 1
item10.dat.cast <- dcast(setDT(wall.final),
                       formula   = CK_Cadmus_ID + BuildingType + HomeYearBuilt ~ rvalue.bins,
                       value.var = 'count')
...
#summarize --SF only
wall.sum <- summarise(group_by(item10.dat.cast, BuildingType, HomeYearBuilt)
                      ,sampleSize      = sum(length(unique(CK_Cadmus_ID)))
                      ,sampleSizeNoNA  = sum(length(unique(CK_Cadmus_ID))) - sum(`NA`)
                      ...
                      ,rGT22.percent   = 0#sum(RGT22) / sampleSizeNoNA
)
#Check to make sure they add to 1, 2, 3, or 4
sum(wall.sum[,c(5:9)])
