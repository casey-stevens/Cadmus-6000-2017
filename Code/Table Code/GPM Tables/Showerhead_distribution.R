...
meas.uncertainty <- meas.uncertainty[which(names(meas.uncertainty) %in% c("Weir.or.Bag"
                                                                          ,"Fixture"
                                                                          ,"Standard.Error.in.Mean.Calibration.Factor"
                                                                          ,"CV.of.Calibration.Factor"))]

water.dat <- left_join(water.dat0, meas.uncertainty, by = c("Weir.or.Bag","Fixture"))
water.dat$Measurement_SE <- (water.dat$Standard.Error.in.Mean.Calibration.Factor)
...
item106.dat4 <- item106.dat3 %>% group_by(CK_Cadmus_ID, BuildingType, State, Weir.or.Bag) %>%
                          ...
                          ,CV_Measurement = sqrt(mean(CV.of.Calibration.Factor^2, na.rm = T)))
...
item106.final$CV_all <- sqrt(item106.final$CV_sampling^2 + item106.final$CV_measurement^2)
item106.final$SE_overall <- item106.final$CV_all * item106.final$w.percent


item106.final <- item106.final %>% left_join(n_total)
item106.final$EB_overall <- item106.final$SE_overall * qt(1-(1-.9)/2,item106.final$n_total-length(unique(item106.final$GPM_bins)))
item106.final$EB_sampling <- item106.final$w.SE * qt(1-(1-.9)/2,item106.final$n_total-length(unique(item106.final$GPM_bins)))
item106.final$var_overall <- (item106.final$SE_overall)^2
item106.final$var_sampling <- (item106.final$w.SE)^2

item106.final$EB_measurement <- sqrt(item106.final$SE_overall^2 - item106.final$var_sampling) * qt(1-(1-.9)/2,item106.final$n)
