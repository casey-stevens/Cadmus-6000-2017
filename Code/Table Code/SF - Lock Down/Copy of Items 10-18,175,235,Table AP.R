...
item10.merge <- left_join(rbsa.dat, item10.dat)
item10.merge <- item10.merge[which(!is.na(item10.merge$total.r.val)),]

item10.merge$total.r.val <- item10.merge$total.r.val #* item10.merge$Wall.Cavity.Insulation.Condition.1

#Bin R values -- SF only
item10.merge$rvalue.bins <- "Unknown"
item10.merge$rvalue.bins[which(item10.merge$total.r.val ==  0)] <- "R0"
item10.merge$rvalue.bins[which(item10.merge$total.r.val >   0  & item10.merge$total.r.val < 11)]  <- "R1.R10"
item10.merge$rvalue.bins[which(item10.merge$total.r.val >= 11  & item10.merge$total.r.val < 17)]  <- "R11.R16"
item10.merge$rvalue.bins[which(item10.merge$total.r.val >= 17  & item10.merge$total.r.val < 23)]  <- "R17.R22"
item10.merge$rvalue.bins[which(item10.merge$total.r.val >= 22)] <- "RGT22"
unique(item10.merge$rvalue.bins)

item10.data <- weightedData(unique(item10.merge[-which(colnames(item10.merge) %in% c("CK_SiteID"
                                                                                     ,"PK_Envelope_ID"
                                                                                     ,"Category"
                                                                                     ,"Wall.Type"
                                                                                     ,"Wall.Area"
...
#weight the u factor per home -- where weights are the wall area within home
# item10.weightedU <- summarise(group_by(item10.data, CK_Cadmus_ID, Wall.Type, rvalue.bins)
#                        ,aveUval = sum(Wall.Area * Wall.Cavity.Insulation.Condition.1 * uvalue) / sum(Wall.Area * Wall.Cavity.Insulation.Condition.1)
# )
...
sort(unique(item10.weightedU$aveRval))
