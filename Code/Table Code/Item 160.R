...
length(unique(item160.dat$CK_Cadmus_ID))#547

#trim white space from cadmus IDs
item160.dat$CK_Cadmus_ID <- trimws(item160.dat$CK_Cadmus_ID)
...
length(unique(item160.dat2$CK_Cadmus_ID))#506
...
length(unique(item160.dat3$CK_Cadmus_ID))#501
...
#make into dataframe
item160.dat4 <- as.data.frame(left_join(item160.dat3, clean.insul, by = c("CK_Cadmus_ID", "TMP_ID"))
                             , stringsAsFactors = F) 
# warning here is OK

item160.merge <- left_join(item160.dat4, item160.mechanical, by = "CK_Cadmus_ID")
item160.merge1 <- item160.merge[which(item160.merge$Heating.Fuel == "Electric"),]

###########################
# Cleaning inches and rvalue information
###########################
# rename columns
item160.merge1$inches1 <- as.numeric(as.character(item160.merge1$V1.x)) # warning here is OK
item160.merge1$inches2 <- as.numeric(as.character(item160.merge1$V1.y)) # warning here is OK

item160.merge1$rvalues1 <- item160.merge1$Wall.Cavity.Insulation.Type.1
item160.merge1$rvalues2 <- item160.merge1$Wall.Cavity.Insulation.Type.2

#check uniques
unique(item160.merge1$rvalues1)
unique(item160.merge1$rvalues2)

#fix names that are not in R value table
item160.merge1$rvalues1[which(item160.merge1$rvalues1 == "Fiberglass or mineral wool batts")] <- "Mineral wool batts"
item160.merge1$rvalues1[which(item160.merge1$rvalues1 == "Unknown fiberglass")]               <- "Unknown"
item160.merge1$rvalues2[which(item160.merge1$rvalues2 == "Extruded polystyrene (blue)")]      <- "Extruded polystyrene foam board"
item160.merge1$rvalues2[which(item160.merge1$rvalues2 == "N/A")]                              <- NA
###########################
# End cleaning step
###########################
...
for (i in 1:length(rvals$Type.of.Insulation)){
  item160.merge1$rvalues1[which(item160.merge1$rvalues1 == rvals$Type.of.Insulation[i])] <- rvals$`Avg..R-Value.Per.Inch`[i]
  item160.merge1$rvalues2[which(item160.merge1$rvalues2 == rvals$Type.of.Insulation[i])] <- rvals$`Avg..R-Value.Per.Inch`[i]
}
###########################
# End Replace Step
###########################

item160.dat5 <- item160.merge1
...
item160.dat5$total.r.val <- (as.numeric(as.character(item160.dat5$rvalues1)) * item160.dat5$inches1) + (as.numeric(as.character(item160.dat5$rvalues2)) * item160.dat5$inches2)
...
#summarize --SF only
item160.sum <- summarise(group_by(item160.SF.dat, BuildingType, Wall.Type)
...
item160.sum.allLevels <- summarise(group_by(item160.SF.dat, BuildingType, Wall.Type)
...
item160.final <- data.frame("BuildingType" = item160.sum$BuildingType
                            ,"Wall.Type" = item160.sum$Wall.Type
                            ,"Percent.R0" = item160.sum$r0.percent
                            ,"SE.R0" = item160.sum$r0.se
                            ,"Percent.R1.R10" = item160.sum$r1.r10.percent
                            ,"SE.R1.R10" = item160.sum$r1.r10.se
                            ,"Percent.R11.R16" = item160.sum$r11.r16.percent
                            ,"SE.R11.R16" = item160.sum$r11.r16.se
                            ,"Percent.R17.R22" = item160.sum$r17.r22.percent
                            ,"SE.R17.R22" = item160.sum$r17.r22.se
                            ,"Percent.RGT22" = item160.sum$rGT22.percent
                            ,"SE.RGT22" = item160.sum$rGT22.se
